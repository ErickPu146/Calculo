...
const BasicDerivative = () => {
  ...
  const onSubmit = (data) => {
    const { base, exponente } = data
    console.log("🚀 ~ onSubmit ~ data:", data)
    const expresion = base.trim() + exponente.trim()
    setResult(calculateDerivative(expresion));
  };
    ...
    <>
      <Form data-aos="fade-up" onSubmit={handleSubmit(onSubmit)}>
        <Row className="row-gap-3 justify-content-center">
          <Col md={6}>
            <label>Ingrese una expresión básica:</label>
            <Controller
              name="base"
              control={control}
              render={({ field }) => (
                <Form.Control placeholder="Ejemplo: 4x" type="text" size="lg" {...field} required />
              )}
            />
          </Col>
          <Col md={6}>
            <label>Ingrese el exponente del número:</label>
            <Controller
              name="exponente"
              control={control}
              render={({ field }) => (
